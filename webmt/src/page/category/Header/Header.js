...
class Header extends React.Component {
    ...
    renderTabs(){
        ...
        for(let key in tabs) {
            ...
            array.push(
                <div className={cls} key={item.key} onClick={()=>this.changeTab(key)}>
                    {item.text}
                </div>
            )
        }
...
export default connect(
    state =>({
        tabs: state.categroyReducer.tabs,
        activeKey: state.categroyReducer.activeKey,
        filterData: state.categroyReducer.filterData,
        closePanel: state.categroyReducer.closePanel,
    })
)(Header);
